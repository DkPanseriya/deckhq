...
| [06-RELAY-DESIGN.md](06-RELAY-DESIGN.md) | WP-32/33/34: the relay protocol, the eight design decisions, what it refuses to claim, what is left for the owner. Design only — no code, nothing run | Building WP-32 |
| [07-STUDIO-DESIGN.md](07-STUDIO-DESIGN.md) | WP-66 to WP-71: Studio, an opt-in mode that grills an idea into a blueprint, a roster and a kanban, and hires **real** runtime sessions to work it. The eleven-step loop, the consent for writing into a project, the board schema, the handover gate, and what its budget stop cannot do. Design only — no code, nothing run | Building WP-66 |
...
| [MEDIA.md](MEDIA.md) | What every image in this repository is — a `capture` of DeckHQ running, a `golden` render of a fixture, or a designer's `illustration` of a specification — which ones are stale, and the rule the site build enforces so a mockup is never shown as a screenshot | Publishing any image on the site or in the README |
