...
## 80. WP-03's hero GIF: the floor did not walk, and there was no encoder to record it with
...
| | Before | After |
|---|---|---|
| Frames in which anything moved after the turn ended | **1** | **41** |
| Pixels changed in that frame | **431,956** | 887–1,544 per frame |
| Bounding box of the motion | whole floor | marches x=471 → x=59 |
...
the file from 438 KB to 240 KB, because that one frame was the only
...
**Result.** `docs/media/hero.gif`, 1200×750, 60 frames at 10 fps, 6.0 s,
**240 KB** against the 3 MB budget. Verified by parsing the encoded file
back: 60 image descriptors, a uniform 10 cs delay, a trailer at the last
byte, and sub-image origins that march x=471 → x=59 over frames 1–41 and
