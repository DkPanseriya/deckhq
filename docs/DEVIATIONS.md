...
§133. The run also found that the real payload carries no `tool_use_id`, which
§133.4 records and which changed the route's fallback key.*
...
September: §133. The deltas arrive as described; §133.5 has the timings and the
...
## 133. Live runs — WP-19 and WP-09 were run against a logged-in Claude Code, and `claude -p` turned out to be the wrong surface
...
them, and §133.2 is the list of what moved.

### 133.1 The setup, and what it never touched
...
### 133.2 `claude -p` cannot raise a permission prompt at all, and that cost four turns
...
### 133.4 The payload is not §86.2's payload
...
### 133.6 What this cost, and the budget it broke
...
first `PermissionRequest` ever fired, four of them buying nothing but §133.2.
...
running this should start at §133.2's fourth row and spend one.

### 133.7 One thing not diagnosed
