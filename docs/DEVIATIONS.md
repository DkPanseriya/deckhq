...
## 96. WP-10 — the queue strip and the deck: eight departures from `05` §3, and the two guarded tests that moved
...
### 96.1 The ordering is duplicated on purpose, and pinned by a test
...
### 96.2 `J` and `K` now sort stalls last on the floor too
...
### 96.3 One cursor rule, and the reason it has two forms
...
### 96.4 The chip is ringed only when somebody really is selected; the deck row always has a cursor
...
### 96.5 `Tab` is claimed only when focus is on the floor
...
### 96.6 Five labelled columns, and the MK tag rides inside WHO
...
### 96.7 `1 Reply` opens the panel; `2` and `3` do not
...
### 96.8 The strip has no scroller, and the accent moved off the clock
...
### 96.9 Two guarded tests moved, both in the strengthening direction
...
### 96.10 What this cost, and what is not covered
