...
| F5 first run is a modal | **Closed.** WP-13 deleted the modal for three coach marks on real elements, reading in 10.2 s against a 15 s budget, `Escape` skipping permanently, and demo actors for an empty machine who leave within one poll of a real session arriving (§108). Two of the three marks point at the whole canvas rather than at the office and an agent, because the renderer exposes no geometry — WP-57 item 1, §108.1. |
...
| New (4 Sep): `main` is red on Ubuntu and macOS | `test/unit/plugin-hook.test.mjs`'s "a .cmd shim is run as an argument to the interpreter" asserts a `cmd.exe` resolution that only happens on `win32` and is not platform-guarded. It is one test and it holds the P0 gate shut. |
| New (4 Sep): no completed CI run for `HEAD` | Every push since the WP-10 merge was cancelled by the next merge. The last run to finish, at that merge, failed on Ubuntu, macOS and the goldens job. Nothing merged after it has been through CI at all. |

Test count is now **1265**, lint and format clean on this machine.
